            ...
            <div>
              <div id="dropdownAction" class="z-10 hidden bg-white divide-y divide-gray-100 rounded-lg shadow w-44">
                <ul class="text-sm text-gray-700" aria-labelledby="dropdownActionButton">
                  <li>
                    <button data-modal-target="addBlogsModal" data-modal-show="addBlogsModal" class="block px-4 py-2 w-full hover:bg-gray-100">Adicionar Blogs</button>
                  </li>
                  <li>
            ...
            <?php foreach ($blogs as $blogs) { ?>
              <tr class="bg-white border-b">
                <th scope="row" class="flex items-center px-6 py-4 text-gray-900 whitespace-nowrap">
                  <img class='lazy w-10' data-src='./uploads/blogs/<?php echo $blogs['img']; ?>'>
                  <div class="pl-3">
                    <div class="text-base font-semibold"><?php echo $blogs['name']; ?></div>
                  </div>
                </th>
                <th class="px-6 py-3">
                  <?php echo $blogs['categorie_type']; ?>
                </th>
                <td class="px-6 py-4">
                  <a href="./editar_blogs.php?id=<?php echo $blogs['id']; ?>" type="button" class="font-medium text-blue-600 hover:underline">Editar</a>
                  <a href="./controllers/delete_blogs.php?id=<?php echo $blogs['id']; ?>" type="button" class="font-medium text-red-600 hover:underline">Excluir</a>
                </td>
              </tr>
            <?php } ?>
          </tbody>
        </table>
        <?php include "./components/modal_add_blogs.php"; ?>
      </div>
    </div>
  </div>
  <script>
